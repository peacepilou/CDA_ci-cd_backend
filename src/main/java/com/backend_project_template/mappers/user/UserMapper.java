...
public record UserMapper() {
  public static User toDomain(UserEntity userEntity) {
    ...
    if (userEntity.getOrders() != null) {
      userEntity.getOrders().forEach(orderEntity -> user.getOrders().add(OrderMapper.toDomain(orderEntity, user)));
    }
  ...
  public static UserEntity toEntity(User user) {
    List<OrderEntity> orderEntities = user.getOrders().stream().map(OrderMapper::toEntity).collect(Collectors.toCollection(ArrayList::new));
